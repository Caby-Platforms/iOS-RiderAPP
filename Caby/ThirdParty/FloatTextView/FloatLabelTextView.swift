...
@IBDesignable class FloatLabelTextView: UITextView {
	...
	var placeholderTextColor = UIColor.darkGray    //.withAlphaComponent(0.65)
	...
	var titleFont:UIFont = UIFont.systemFont(ofSize: 12.0) { //getFont(AppFont.Pangram_Light.rawValue, size: 12.0) {
		didSet {
	...
	@IBInspectable var hint:String = "" {
		didSet {
			title.text = hint.localized
			title.sizeToFit()
			var r = title.frame
			r.size.width = frame.size.width
			title.frame = r
			hintLabel.text = hint.localized
			hintLabel.sizeToFit()
		}
	}
	...
	override func layoutSubviews() {
		...
		let r               = textRect()
        hintLabel.font      = UIFont(name: (self.font?.fontName)!, size: (self.font?.pointSize)! )
        hintLabel.textColor = placeholderTextColor
//        title.font          = getFont(AppFont.Pangram_Light.rawValue, size: (self.font?.pointSize)! - 3.0 ) // my update
        hintLabel.frame     = CGRect(x:r.origin.x, y:r.origin.y - 5 , width:self.frame.size.width, height:hintLabel.frame.size.height)
	...
	private func setup() {
		...
		// Placeholder label
		hintLabel.font = UIFont(name: (self.font?.fontName)!, size: (self.font?.pointSize)! * ((kScreenWidth / 320.0))) //font
		hintLabel.text = hint.localized
...
//        title.font = getFont(AppFont.Pangram_Light.rawValue, size: (self.font?.pointSize)! - 3.0 ) // my update
		...
		title.textColor = titleTextColour
		title.backgroundColor = backgroundColor // UIColor.black //change by me at 13-07-17
		if !hint.isEmpty {
			title.text = hint
			title.sizeToFit()
		}
